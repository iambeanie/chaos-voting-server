...
static int currentMode = DeathRollState::start;
...
static void OnStart()
{
	...
	scaleForm = 0;
	currentMode = DeathRollState::start;
	lastModeTime = 0;
	nextModeTime = 0;

	while (currentMode < DeathRollState::cleanup)
	{
		WAIT(0);

		if (currentMode > DeathRollState::animation)
		{
		...
		if (currentMode != DeathRollState::cleanup) {
			SET_PLAYER_INVINCIBLE(playerPed, true);
		}

		switch (currentMode)
		{
		case DeathRollState::animation: // Play either the suicide animation or an explosion if in vehicle
			...
			nextModeTime = 0;
			break;
		case DeathRollState::soundStart: // Apply Screen Effect
			...
			break;
		case DeathRollState::overlay: // 2 Seconds later, Show Fake Wasted Screen Message
			...
			break;
		case DeathRollState::cleanup: // Remove all Effects, so you dont have to see this for the rest of the duration
			Vehicle veh = GET_VEHICLE_PED_IS_IN(playerPed, false);
...
static RegisterEffect registerEffect(EFFECT_DEATH_ROLL, OnStart, EffectInfo
	{
		.Name = "Death Roll",
		.Id = "player_death_roll"
	}
);